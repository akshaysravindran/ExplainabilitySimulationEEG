...
import psutil
from utils_explanationF import attribute_image_features,calc_measures   
...
indices= np.array(range(1,40000,1))
kf = KFold(n_splits=5)
fold=0
for train_index, test_index in kf.split(indices):
    ...
    # Files
        filename                  = 'Spectral_1_%g.mat'%iteration 
        BW_Tall                   = '%s_weights_%d.pt'%(filename[:-4],fold)
        file = 'Spectral/Data/'+filename
        # Use GPU if available
        
        # Load the data
        X, Y, GT, fake_evoked=read_data(file, iteration, siomat_flag=0)        
        ...
        # test_epoch_loss, test_epoch_accuracy = validate_model(
        ...
        for images, labels, true_loc in dataiter:   
            ...
            for i in range(8):
                ...
                Measures1, count            = calc_measures(grad, locations , gt_flag=1, abs_condition = 0, prctile_val=95)
                Measuresrand, countrand     = calc_measures(gradrand, grad, gt_flag=0, abs_condition = 0,  prctile_val=95)            
                df1                         = {(name+'-True'):np.mean(Measures1,axis=0),(name+'-Rand'):np.mean(Measuresrand,axis=0), (name+'nan'):count}  
                df1                         = pd.DataFrame(df1)   
                df                          = pd.concat([df,df1],axis=1) 
                print(name)
                del gradcam,grad,gradrand,Measures1,Measuresrand
                torch.cuda.empty_cache()
            ...
            for i in range(5):
                ...
                df1                         = pd.DataFrame(df1) 
                df                          = pd.concat([df,df1],axis=1) 
                del cam,grad,gradrand,Measures1,Measuresrand
                torch.cuda.empty_cache()
                print(name)
            Performance_Randomized                               = Performance_Randomized.append(df)      
            ...
            for layer in reversed(Layer_list):
                ...
                for i in range(8):  
                        ...
                        Measuresrand, countrand     = calc_measures(gradrand , grad,  gt_flag=0, abs_condition = 0,  prctile_val=95)                    
                        df1={(str(layer)[:6] + name ):np.mean(Measuresrand,axis=0)}   
                        df1                         = pd.DataFrame(df1)    
                        df                          = pd.concat([df,df1],axis=1)  
                        print(name)
                        del gradcam_orig,grad,gradrand,gradcam_shuffled,Measuresrand,df1
                        torch.cuda.empty_cache()
                ...
                for i in range(5):
                        ...
                        Measuresrand, countrand     = calc_measures(grad, gradrand , gt_flag=0, abs_condition = 0,  prctile_val=95)                    
                        df1={(str(layer)[:6] + name ):np.mean(Measuresrand,axis=0)}     
                        
                        df1                         = pd.DataFrame(df1)    
                        df                          = pd.concat([df,df1],axis=1)    
                        del cam,grad,gradrand,cam_rand,Measuresrand
                        torch.cuda.empty_cache()
                        print(name)
            Performance_Weight_Randomized                        = Performance_Weight_Randomized.append(df)
            ...
            del images, labels, locations, input,rand_label, model_randomize
    
            # print('Batch %d done   : Time elapsed: '%batch + str(np.round((end - start)/60,1)) + ' minutes done' )
            # print('Batch %d done   : Time elapsed: '%batch + str(np.round((end - start)/60,1)) + ' minutes done' )
        ...
        np.savez('%sv_%d.npz'%(BW_Tall[:-11],fold), Performance_Weight_Randomized=Performance_Weight_Randomized,
                                           ...
                                           allow_pickle=True)        
        del model, Performance_Weight_Randomized,Performance_Randomized, PERF,dataiter, test_dataset,Performance_SpatialRandomized,Performance_Weight_Randomized_Spatial
